...
const SpringModal = ({ isOpen, setIsOpen, questionList, setQuestionList, user, data }) => {
    ...
    const handleSubmit = () => {
        ...
        // Using regular expression to extract the desired part from the path
        var matches = url.match(/\/([^\/]+)\/([^\/]+)/);

        // Extracting the desired part ("html-and-css")
        var desiredPart = matches && matches[2];
        console.log(desiredPart,url,matches)
